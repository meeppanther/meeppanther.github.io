...
function getThrees(arrayItems) {
    ...
    for (index = 0; index < arrayItems.length; index++) {
        ...
        allItems += "/";
        allItems += arrayItems[index].id;
        allItems += "/mmr?playlist=13'></iframe></div>";// 2's is 11, 3's is 13 and tourney is 34
        allItems += "</div>";
        if (index % 6 == 5) {
            allItems += "</div>";
        }
...
document.getElementById("Threes").innerHTML = getThrees(personalAccounts);
